...
bool DumpFloatMemoryDataSet(char *name, float* device_values, int nb_data ){

    //
    // ON ALLOUE LA ZONE MEMOIRE POUR RECUPERE LES DONNEES PROVENANT DU GPU
    //
    printf("(II) DumpFloatMemoryDataSet(%s, 0x%8.8X, %d)\n", name, device_values, nb_data);
    cudaError_t Status;
    float* host_values;
    CUDA_MALLOC_HOST(&host_values, nb_data);
